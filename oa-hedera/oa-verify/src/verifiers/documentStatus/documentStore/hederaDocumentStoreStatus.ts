...
import { OpenAttestationHederaDocumentStoreStatusCode, Reason } from "../../../types/error";
import { CodedError } from "../../../common/error";
import { withCodedErrorHandler } from "../../../common/errorHandler";
import { decodeError, isRevokedOnDocumentStore, getGas } from "../utils";
...
import {
  ...
  OpenAttestationHederaDocumentStoreStatusFragment,
  ValidDocumentStoreDataV3,
  ValidDocumentStoreIssuanceStatusArray,
} from "./hederaDocumentStoreStatus.type";
import { Client, ContractId } from "@hashgraph/sdk";

const name = "OpenAttestationHederaDocumentStoreStatus";
const type: VerificationFragmentType = "DOCUMENT_STATUS";
type VerifierType = Verifier<OpenAttestationHederaDocumentStoreStatusFragment>;
  ...
  return data.issuers.map((issuer) => {
    ...
    if (!documentStoreAddress)
      throw new CodedError(
        ...
        OpenAttestationHederaDocumentStoreStatusCode.INVALID_ISSUERS,
        OpenAttestationHederaDocumentStoreStatusCode[OpenAttestationHederaDocumentStoreStatusCode.INVALID_ISSUERS]
      );
...
export const isIssuedOnDocumentStore = async ({
  ...
  try {
    ...
    const issued = await documentStoreContract.isIssued(merkleRoot, getGas());
    return issued
      ? {
          issued: true,
      ...
      : {
          ...
          reason: {
            ...
            code: OpenAttestationHederaDocumentStoreStatusCode.DOCUMENT_NOT_ISSUED,
            codeString:
              OpenAttestationHederaDocumentStoreStatusCode[
                OpenAttestationHederaDocumentStoreStatusCode.DOCUMENT_NOT_ISSUED
              ],
          },
        };
  } catch (error) {
...
const skip: VerifierType["skip"] = async () => {
  return {
    ...
    name,
    reason: {
      code: OpenAttestationHederaDocumentStoreStatusCode.SKIPPED,
      codeString: OpenAttestationHederaDocumentStoreStatusCode[OpenAttestationHederaDocumentStoreStatusCode.SKIPPED],
...
const verifyV2 = async (
...
): Promise<OpenAttestationHederaDocumentStoreStatusFragment> => {
  ...
  const issuanceStatuses = await Promise.all(
    documentStores.map((documentStore) =>
      isIssuedOnDocumentStore({ documentStore, merkleRoot, client: options.client })
    )
  );
  ...
  throw new CodedError(
    ...
    OpenAttestationHederaDocumentStoreStatusCode.UNEXPECTED_ERROR,
    "UNEXPECTED_ERROR"
  );
};
...
const verifyV3 = async (
  ...
  const issuance = await isIssuedOnDocumentStore({ documentStore, merkleRoot, client: options.client });
...
const verify: VerifierType["verify"] = async (document, options) => {
  ...
  throw new CodedError(
    ...
    OpenAttestationHederaDocumentStoreStatusCode.UNRECOGNIZED_DOCUMENT,
    OpenAttestationHederaDocumentStoreStatusCode[OpenAttestationHederaDocumentStoreStatusCode.UNRECOGNIZED_DOCUMENT]
  );
};

export const openAttestationHederaDocumentStoreStatus: VerifierType = {
  skip,
  test,
  verify: withCodedErrorHandler(verify, {
    name,
    type,
    unexpectedErrorCode: OpenAttestationHederaDocumentStoreStatusCode.UNEXPECTED_ERROR,
    unexpectedErrorString:
      OpenAttestationHederaDocumentStoreStatusCode[OpenAttestationHederaDocumentStoreStatusCode.UNEXPECTED_ERROR],
  }),
};
