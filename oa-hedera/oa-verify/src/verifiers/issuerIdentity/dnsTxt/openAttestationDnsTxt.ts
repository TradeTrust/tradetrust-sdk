...
import { getDocumentStoreRecords } from "@tianhr/dnsprove";
...
const resolveIssuerIdentity = async (
  ...
  const smartContractAddress_ = addressToContractId(smartContractAddress).toString();
  const matchingRecord = records.find(
    (record) =>
      record.addr.toLowerCase() === smartContractAddress_ &&
      record.netId === "3" &&
  ...
  return matchingRecord
    ...
    : {
        ...
        value: smartContractAddress_,
        reason: {
          message: `Matching DNS record not found for ${smartContractAddress_}`,
