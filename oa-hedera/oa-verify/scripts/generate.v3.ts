  ...
  openAttestationMetadata: {
    ...
    proof: {
      ...
      revocation: {
        ...
        location: hederaDocumentConfig.documentStore,
      },
    },
  },
};
...
const generateRevocationStore = async ({ logMessage, document, outLocations }: GenerateRevocationStoreArgs) => {
  ...
  const cmdRevoke = `oa document-store revoke -h 0x${signed.proof.merkleRoot} -a ${hederaDocumentConfig.documentStore} -k ${hederaDocumentConfig.wallet.key} -n ${hederaDocumentConfig.network}`;
  execSync(cmdRevoke, { timeout: hederaDocumentConfig.timeout });
};
...
const generateDocumentStore = async () => {
  ...
  const cmdIssue1 = `oa document-store issue -h 0x${issuedBaseDocumentStoreDocument.proof.merkleRoot} -a ${hederaDocumentConfig.documentStore} -k ${hederaDocumentConfig.wallet.key} -n ${hederaDocumentConfig.network}`;
  execSync(cmdIssue1, { timeout: hederaDocumentConfig.timeout });
  info("Issuing document(2/3)...");
  const cmdIssue2 = `oa document-store issue -h 0x${revokedBaseDocumentStoreDocument.proof.merkleRoot} -a ${hederaDocumentConfig.documentStore} -k ${hederaDocumentConfig.wallet.key} -n ${hederaDocumentConfig.network}`;
  execSync(cmdIssue2, { timeout: hederaDocumentConfig.timeout });
  info("Issuing document(3/3)...");
  const cmdIssue3 = `oa document-store issue -h 0x${invalidDnsDocumentStoreDocument.proof.merkleRoot} -a ${hederaDocumentConfig.documentStore} -k ${hederaDocumentConfig.wallet.key} -n ${hederaDocumentConfig.network}`;
  execSync(cmdIssue3, { timeout: hederaDocumentConfig.timeout });
  info("Revoking document...");
  const cmdRevoke = `oa document-store revoke -h 0x${revokedBaseDocumentStoreDocument.proof.merkleRoot} -a ${hederaDocumentConfig.documentStore} -k ${hederaDocumentConfig.wallet.key} -n ${hederaDocumentConfig.network}`;
  execSync(cmdRevoke, { timeout: hederaDocumentConfig.timeout });
};
...
const generateTokenRegistry = async () => {
  ...
  const cmdIssue1 = `oa token-registry mint -a ${hederaDocumentConfig.tokenRegistry} --tokenId 0x${issuedBaseTokenRegistryDocument.proof.merkleRoot} --to ${hederaDocumentConfig.wallet.address} -k ${hederaDocumentConfig.wallet.key} -n ${hederaDocumentConfig.network}`;
  info("Issuing document(1/2)...");
  execSync(cmdIssue1, { timeout: hederaDocumentConfig.timeout });
  info("Issuing document(2/2)...");
  const cmdIssue2 = `oa token-registry mint -a ${hederaDocumentConfig.tokenRegistry} --tokenId 0x${invalidDnsTokenRegistryDocument.proof.merkleRoot} --to ${hederaDocumentConfig.wallet.address} -k ${hederaDocumentConfig.wallet.key} -n ${hederaDocumentConfig.network}`;
  execSync(cmdIssue2, { timeout: hederaDocumentConfig.timeout });
};
