...
import { readOpenAttestationFile } from "../utils/disk";
...
export const create = async ({
  ...
  const walletStr = await readFile(encryptedWalletPath);
  const file = JSON.parse(walletStr);
  info(`Wallet detected at ${encryptedWalletPath}`);
  ...
  if (hasDid) {
    ...
    // did:hedera:testnet:7c38oC4ytrYDGCqsaZ1AXt7ZPQ8etzfwaxoKjfJNzfoc;hedera:testnet:fid=0.0.1

    const addressBook = '0.0.24352';
    const pubKey = file.operator.publicKey;
    const did = new HcsDid("testnet", pubKey, FileId.fromString(addressBook));
